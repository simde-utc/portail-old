<div class="well">
  <div class="row-fluid" style="text-align:center;">
    <div class="span4">
    	<h1>Etudiants</h1>
    	<div><img src="/images/icones/job.jpg" style="witdh:90px; height:70px;"></div>
      <br/>
    	<div><a href="<?php echo url_for('infojob/offres') ?>" class="btn btn-success">Voir les offres</a></div>
    </div>
    
    <div class="span4">
      <h1>Entreprise et particuliers</h1>
      <div><img src="/images/icones/entreprise.jpg" style="line-height:normal;witdh:90px; height:70px;"></div>
	    <br/>
	   	<div><a href="<?php echo url_for('infojob/new') ?>" class="btn btn-warning">Déposer une offre</a></div>
    </div>
    
    <div class="span4">
      <h1>Etudiants</h1>
	    <div><img src="/images/icones/etudiant.jpg" style="line-height:normal;witdh:90px; height:70px;"></div>
      <br/>
    	<div><a href="<?php echo url_for('infojob/monprofil') ?>" class="btn btn-info">Gérer mon compte</a></div>
    </div>
  </div>
</div>
